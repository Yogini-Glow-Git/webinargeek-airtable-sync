...
/**
 * Sucht eine bestehende Subscription per E-Mail + Broadcast (fuer den
 * "already_registered"-Fall, wo die POST-Antwort keinen watch_link liefert -
 * es gibt keinen dedizierten GET /broadcasts/{id}/subscriptions-Endpoint,
 * laut WebinarGeek-API-Doku ist der generische GET /subscriptions mit
 * email+broadcast_id-Filter der richtige Weg).
 */
async function findSubscriptionByEmail(email, broadcastId) {
  const params = new URLSearchParams({ email, broadcast_id: String(broadcastId) });
  const data = await wgFetch(`${WG_API}/subscriptions?${params.toString()}`);
...
export async function registerForNextBroadcast(contact, { skipConfirmationMail = false, utm = null } = {}) {
  ...
  try {
    ...
    // POST-Antwort liefert bei diesem Fehlerpfad keinen watch_link mit -
    // separat nachschlagen, damit der persoenliche Zugangslink trotzdem
    // zurueckgegeben werden kann (z.B. fuer den Kalender-Eintrag auf /danke).
    try {
      sub = await findSubscriptionByEmail(email, broadcast.id);
    } catch (lookupErr) {
      console.warn(`[webinarRegistration] watch_link-Nachschlag fuer ${email} fehlgeschlagen:`, lookupErr?.message);
    }
  }
  ...
  return {
    ...
    watchLink: sub?.watch_link ?? null, // persoenlicher Zugangslink zum Webinar-Raum
    easy2Tag,
  };
}
